...
//уменьшение размеров текстур травы
grassColorTexture.repeat.set(8, 8);
grassAmbientOcclusionTexture.repeat.set(8, 8);
grassNormalTexture.repeat.set(8, 8);
grassRoughnessTexture.repeat.set(8, 8);
//включение репита в одну сторону
grassColorTexture.wrapS = THREE.RepeatWrapping;
grassAmbientOcclusionTexture.wrapS = THREE.RepeatWrapping;
grassNormalTexture.wrapS = THREE.RepeatWrapping;
grassRoughnessTexture.wrapS = THREE.RepeatWrapping;

//включение репита во вторую сторону
...
const door = new THREE.Mesh(
    //3 и 4 параметры это количество полигонов. Большое количество необходимо для работы displacementMap
    new THREE.PlaneBufferGeometry(2.2, 2.2, 100, 100),
    new MeshStandardMaterial({
        map: doorColorTexture,
        //Прозрачность для работы альфы
        transparent: true,
...
//Установка uv координат для работы aoMap
...
for (let i = 0; i < 50; i++) {
    //4 чтобы не заползать на модель дома
...
const tick = () => {
    ...
    //TODO Посмотреть на прыгающем шарике как работает двойной синус
    ghost2.position.y = Math.sin(ghost2Angle * 4) + Math.sin(ghost2Angle * 4);
    ...
    ghost3.position.y = Math.sin(ghost2Angle * 5) + Math.sin(ghost2Angle * 2.5);
